...
class Users{
	...
	public function update_user($first_name, $last_name, $phone, $account, $address, $gender, $blood, $bio, $image_location,$admin, $id){
		...
		$query->bindValue(7, $blood);
		$query->bindValue(8, $bio);
		$query->bindValue(9, $image_location);
		$query->bindValue(10, $admin);
    ...
    public function fetch_info($what, $field, $value){
		...
		$allowed = array('id', 'username', 'first_name', 'last_name', 'phone', 'account', 'address', 'gender', 'blood', 'bio', 'LastLogin', 'admin', 'email'); // I have only added few, but you can add more. However do not add 'password' eventhough the parameters will only be given by you and not the user, in our system.
	...
	public function fetch_info1($what1, $field1, $value1){

		$allowed1 = array('id', 'from', 'to', 'message', 'is_read', 'time'); // I have only added few, but you can add more. However do not add 'password' eventhough the parameters will only be given by you and not the user, in our system.
		if (!in_array($what1, $allowed1, true) || !in_array($field1, $allowed1, true)) {
		    throw new InvalidArgumentException;
		}else{
		
			$query = $this->db->prepare("SELECT $what1 FROM `messages` WHERE $field1 = ?");
	...
	public function confirm_recover($email){
		...
		try{
			...
			mail($email, 'Recover Password', "Hello " . $username. ",\r\nPlease click the link below:\r\n\r\nhttp://www.shometi.6te.net/recover.php?email=" . $email . "&generated_string=" . $generated_string . "\r\n\r\n We will generate a new password for you and send it back to your email.\r\n\r\n-- 11 member team \r\n\r\n Bodal Kote, Chatkhil, Noakhali");			
	...
	public function account_exists($account) {

		$query = $this->db->prepare("SELECT COUNT(`id`) FROM `users` WHERE `account`= ?");
	...
	public function register($username, $password, $email){
		...
		try{
			...
			mail($email, 'Please activate your account', "Hello " . $username. ",\r\nThank you for registering with us. Now your are a honorable member of our shometi. Please visit the link below so we can activate your account:\r\n\r\nhttp://www.shometi.6te.net/activate.php?email=" . $email . "&email_code=" . $email_code . "\r\n\r\n-- 11 member team \r\n\r\n Bodal Kote, Chatkhil, Noakhali");
		}catch(PDOException $e){
			die($e->getMessage());
		}	
	}
